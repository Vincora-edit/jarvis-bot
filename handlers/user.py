...
async def get_tariff_message(user_id: int, show_back: bool = False) -> tuple[str, InlineKeyboardMarkup]:
    ...
    if ai["unlimited"]:
        text += "• AI-запросы сегодня: ∞\n"
    else:
        text += f"• AI-запросы сегодня: {ai['used']}/{ai['limit']}\n"

    if reminders["unlimited"]:
        text += "• Напоминания сегодня: ∞\n"
    else:
        text += f"• Напоминания сегодня: {reminders['used']}/{reminders['limit']}\n"

    text += f"• VPN устройств: {usage_info['vpn_devices']}\n"
